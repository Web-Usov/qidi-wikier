...
const MARKET = /(продам|куплю|обменяю|барахолк|доставка|маркетплейс|авито|(?:ozon(?:\.ru)?|озон)|wildberries|вайлдберриз|валдберрис|вилдберрис|wb\.ru|(?:^|\s)вб(?:\s|$)|\bali(?:express)?\b|aliexpress|(?<![\p{L}\p{N}])али(?:экспресс)?(?![\p{L}\p{N}])|яндекс.{0,8}маркет|market\.yandex|скидк|промокод|за\s*\d[\d\s]*(?:₽|руб))/iu;
