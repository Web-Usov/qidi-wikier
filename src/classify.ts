...
const MARKET = /(продам|куплю|обменяю|барахолк|доставка|маркетплейс|авито|(?:ozon(?:\.ru)?|озон)|wildberries|вайлдберриз|валдберрис|вилдберрис|wb\.ru|(?:^|\s)вб(?:\s|$)|ali(?:express)?|aliexpress|али(?:экспресс)?|яндекс.{0,8}маркет|market\.yandex|скидк|промокод|за\s*\d[\d\s]*(?:₽|руб))/iu;
